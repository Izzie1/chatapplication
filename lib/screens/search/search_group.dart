...
import 'package:funchat/screens/chat/chatpage.dart';
...
class _SearchPageState extends State<SearchPage> {
  ...
  _getCurrentUserNameAndUid()  async {
     await _repository.getCurrentUser().then((user) {
      ...
      _userName = Utils.getUsername(user.email);
    });
  ...
  _initiateSearch() async {
    if(searchEditingController.text.isNotEmpty){
      ...
      await groupMethods.searchByName(searchEditingController.text).then((snapshot) {
  ...
  _joinValueInGroup(String userName, String groupId, String groupName, String admin) async {
    bool value = await groupMethods.isUserJoined(groupId, groupName, userName, _user.uid);
    setState(() {
      _isJoined = value;
    });
  }
  ...
  Widget groupTile(String userName, String groupId, String groupName, String admin){
    ...
    return ListTile(
      ...
      trailing: InkWell(
        onTap: () async {
          await groupMethods.togglingGroupJoin(groupId, groupName, _user.uid, userName);
          if(_isJoined) {
            setState(() {
              _isJoined = !_isJoined;
            });
